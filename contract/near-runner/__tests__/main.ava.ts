...
runner.test("root sets status", async (test, { contract, root }) => {
  let bufferVal = Buffer.from(sha256(Buffer.from("Dorian", "utf-8")));
  let solution = bufferVal.toString("base64");
  test.log(solution);

  // Don't forget to `await` your calls!
  await root.call(contract, "init", { solution });
  let test_solution = await contract.view("get_solution");
  test.log(test_solution);
  // Assert that two things are identical using `test.is`
  test.is(test_solution, solution);
  let result = await contract.view_raw("guess_solution", {
    solution: "Dorian",
  });
  test.log(result);
  test.assert(result.result);

  test.log(contract.accountId);
});
runner.test("root transaction", async (test, { contract, root }) => {
  // Don't forget to `await` your calls!
  await root.call(contract, "init", { solution: "lol" });

  test.throwsAsync(
    root
      .createTransaction(contract)
      .functionCall("init", { solution: "lol2" })
      .signAndSend()
  );
  ...
  test.is(
    ...
    await contract.view("get_solution", {}),
    "lol"
  );
